...
class Enigma:
    ...
    alphabet_sign = ".:;’~`?,!@#$%^&*()[]{}'<>/\-_=+ 0123456789۰۱۲۳۴۵۶۷۸۹"
    persian_alphabet = "اأإةآبپتثجچحخدذرزژسشصضطظعغفقکگلمنوهی"
    alphabets = lower_en_alphabet + persian_alphabet + alphabet_sign + upper_en_alphabet
    plugboard = {}
    count_rotors = ""
    rotors = []
    state_rotor = 0
    ...
    def master_mind(self, x):
        ...
        # print(character,'\n')
        """
            This Here before rotate started 
            plugboard checked 
            if x in plugboard:
                x = plugboard[x]
        """
    ...
    def rotate_rotors(self):
        ...
        if self.state_rotate % (len(self.alphabets) * (self.state_rotor + 1)) == 0:
            ...
            self.rotors[self.state_rotor] = (
                    self.rotors[self.state_rotor][1:] + self.rotors[self.state_rotor][0]
            )
